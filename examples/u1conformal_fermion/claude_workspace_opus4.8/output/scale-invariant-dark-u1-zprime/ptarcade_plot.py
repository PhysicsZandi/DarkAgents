...
chains_dir = str(_HERE / "chains" / "np_model") + "/"
...
with open(_HERE / "ptarcade_bayes.json", "w") as f:
    json.dump(output, f, indent=4)

p_utils.plot_posteriors(
    [chain],
    [params],
    par_names=[[r"$g_D$", r"$y$", r"$\log_{10}(w/\mathrm{GeV})$"]],
    samples_name=["dark $U(1)_D$ CW FOPT"],
    model_name="scale-invariant-dark-u1-zprime",
    plots_dir=str(_HERE) + "/",
    save=True,
    verbose=True,
