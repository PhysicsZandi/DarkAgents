"""MAP-point spectrum vs PTA violins from PTArcade results (dbf template)."""
...
def main():
    bayes = json.load(open(_HERE / "ptarcade_bayes.json"))
    mp = bayes["map_est"]
    # map keys may use '-' instead of '_'
    def get(k):
    ...
    ax.plot(np.log10(f), np.log10(omega), color="crimson", lw=2.2,
            label=r"MAP: $g=%.3f,\ \chi_0=%.3f$ GeV" % (g, chi0))

    ax.set_xlim(-9, -7)
    ax.set_ylim(-11, -6)
    ax.set_xlabel(r"$\log_{10}(f/\mathrm{Hz})$", fontsize=13)
    ax.set_ylabel(r"$\log_{10}(h^2\Omega_{\mathrm{GW}})$", fontsize=13)
    ax.set_title(r"su2cw-dark MAP spectrum (dbf) vs NANOGrav 15yr", fontsize=13)
    ax.legend(loc="lower left", fontsize=10, framealpha=0.9)
    ax.grid(alpha=0.3)
    fig.tight_layout()
    out = _HERE / "pta_ptarcade_spectrum.pdf"
    fig.savefig(out)
    print("wrote", out, "MAP g=", g, "chi0=", chi0,
          "alpha=", res["alpha"], "betaH=", res["beta_H"], "Treh=", res["Treh"],
          "template", TEMPLATE)
